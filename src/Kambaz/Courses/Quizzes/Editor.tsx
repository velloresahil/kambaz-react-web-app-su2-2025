...
export default function QuizEditor() {
  ...
  const [formData, setFormData] = useState<Partial<Quiz>>({
    ...
    maxAttempts: undefined, // keep from earlier change
  ...
  // When enabling "Allow Multiple Attempts", prompt for number; validate; revert if invalid/cancel.
  const handleMultipleAttemptsToggle = (checked: boolean) => {
    if (checked) {
      ...
      const valid =
        Number.isFinite(num) && Number.isInteger(num) && num > 0;
      if (!valid) {
  ...
  // Single save function; pass publish=true to save & publish in ONE request
  const handleSave = async (publish = false) => {
    ...
    const quizData: Quiz = {
      ...
      maxAttempts:
        formData.multipleAttempts ? Number(formData.maxAttempts) : undefined,
    ...
    try {
      if (isNewQuiz) {
        ...
        setFormData(created); // keep local state in sync (has real _id)
      } else {
        ...
        <Tab eventKey="details" title="Details" className="text-danger">
          ...
          <div className="float-end">
            ...
            <Button
              variant="success"
              className="ms-2"
              onClick={() => handleSave(true)} // Save & Publish in one go
            >
              Save and Publish
            </Button>
          </div>
